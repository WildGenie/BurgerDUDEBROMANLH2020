...
class PrintChampSelectInfo(EventProcessor):
    ...
    def handle(self, event: Event):
        ...
        if (
            event.uri.startswith("/lol-champ-select/v1/grid-champions")
            and event_json['selectionStatus']['isBanned']
        ):
        ...
        if (
            event.uri.startswith("/lol-champ-select/v1/summoners")
            and not event_json['isPlaceholder']
        ):
            # print(event_json)
        ...
        if players_dict[0] == {}:
            ...
            for k, v in players_dict.items():
                ...
                temp = {
                    'encryptedId': 0,
                    'mains': [],
                    'username': summoner_json['displayName'],
                }
                ...
                mains_url = (
                    f"https://{REGION}.api.riotgames.com/lol/champion-mastery/v4/champion-masteries/by-summoner/"
                    + summ4json['id']
                )
